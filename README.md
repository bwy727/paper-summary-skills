...
一套面向科研文献管理的 Claude Code 技能（skill）集合，帮助你把一批 markdown（`.md`）文献，自动整理成**可检索、可筛选、可问答**的结构化资料。
...
| 文献名单 | `<问题关键词>.md` | 针对你的提问，整理出可参考的文献清单 |
...
| **summary_archiving** | 「总结文献」「整理文献」 | `notes_done\` 中的 `.md` | `xlsx_summary\paper_archiving.xlsx` |
| **summary_grouping** | 「整理文献标签」 | `notes_done\` 中的 `.md` | `xlsx_summary\keyword_summary.xlsx` ＋ `xlsx_summary\searching_readme.md` |
| **summary_answering** | 「回答我的问题：XXX」 | 你的问题 ＋ 上述两张表 | `xlsx_summary\<问题关键词>.md` |

> **路径约定**：本技能包使用固定的输入/输出文件夹。
> 输入为 `D:\AIacademic\ReadPaper\notes_done`（阅读笔记所在处，递归含其所有子文件夹），
> 输出为 `D:\AIacademic\ReadPaper\xlsx_summary`（`paper_archiving.xlsx`、`keyword_summary.xlsx`、
> `searching_readme.md`、`<问题关键词>.md` 均生成于此）。
> 两个路径以常量 `IN_DIR` / `OUT_DIR` 写在三个脚本中，需要时可用 `--folder` / `--out` 覆盖。
>
> **术语约定**：本文中「文献文件夹」默认指上述输入文件夹；「文献总结表」指 `paper_archiving.xlsx`；「文献分组表」指 `keyword_summary.xlsx`。全文保持一致，不使用同义替换。
...
| 依赖 | 说明 |
| --- | --- |
| Python 3.9+ | 用于读取 `.md` 与读写 `.xlsx` |
| `openpyxl` | 读写 Excel 表格 |

首次运行时，技能会自动检测并安装 `openpyxl`，通常无需手动操作。如需手动安装：
...
1. 把要处理的 markdown（`.md`）文献放进输入文件夹 `D:\AIacademic\ReadPaper\notes_done`（可分子文件夹）。
2. 生成文献总结表：
   ```
   总结文献
   ```
...
所有产出文件都会生成在输出文件夹 **`D:\AIacademic\ReadPaper\xlsx_summary`**。
...
- **默认范围**：`notes_done\`（递归）中的全部 `.md`；也可指定文件夹或单篇文献。
- **增量更新**：若表已存在，仅追加新文献，自动忽略重复条目。
- **列结构**（从左到右）：

  | 列 | 内容 |
  | --- | --- |
  | 文献名 | 论文标题（md 文件名去扩展名与开头的 `[note] ` 前缀） |
  | 发表年份 | 只依据文献本身（md 文件名与笔记正文）判断，不联网检索、不读取原 PDF；无则留空 |
  | 期刊 | 同上，只依据文献本身；无则留空 |
...
- **默认范围**：`notes_done\`（递归）中的全部 `.md`。
- **关键词一致性**：尽可能多地提取关键词，并在不同文献间保持同一表述。
- **表结构**：第一列为文献名，其后每个关键词占一列，文献含该关键词则标 `X`。
- **筛选能力**：表头开启自动筛选，首列与表头冻结。
- **配套说明**：同时在输出文件夹生成 `searching_readme.md`，详述单关键词筛选、多关键词组合筛选与清除筛选的操作步骤。
...
- **检索逻辑**：
  ...
  - **两者都不存在** → 先与你确认确无此二表，并申请直接检索全部 `.md` 来整理名单。
- **输出**：在输出文件夹生成 `<问题关键词>.md`，列出「可直接回答的文献」与「话题相关的文献」两类名单；同时在文献总结表中筛选呈现这些文献对应的行（另存为 `<问题关键词>_文献总结筛选.xlsx`）。若无文献总结表，会先与你确认是否需要生成。
...
- **文献名**：取 md 文件名去掉扩展名与开头的 `[note] ` 前缀（阅读笔记统一命名为 `[note] <论文标题>.md`），支持中英文混合与较长名称；去重以「论文标题（去首尾空格）」为准。三个脚本的 `stem_key()` 均按此规则。
- **跳过的文件**：隐藏文件（以 `.` 开头）、`_work\` 目录，以及技能自身生成的 `searching_readme.md`。
- **路径固定**：输入/输出文件夹见上文「路径约定」，不随当前工作目录变动；`--out` / `--archiving` 只给文件名时会自动落在输出文件夹内。
- **年份与期刊**：只依据文献本身（md 文件名与笔记正文）判断，**不读取原 PDF**，也不联网检索；看不出则留空。
