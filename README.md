...
| **summary_archiving** | 「总结文献」「整理文献」 | `note\` 中的 `.md` | `paper_archiving.xlsx` |
| **summary_grouping** | 「整理文献标签」 | `note\` 中的 `.md` | `keyword_summary.xlsx` ＋ `searching_readme.md` |
| **summary_answering** | 「回答我的问题：XXX」 | 你的问题 ＋ 上述两张表 | `<问题关键词>.md` |
...
1. 在项目根目录建立文献文件夹 `note\`，把要处理的 markdown（`.md`）文献放进去（可分子文件夹）。
2. 生成文献总结表：
   ```
   总结文献
   ```
...
- **默认范围**：`note\`（递归）中的全部 `.md`；也可指定文件夹或单篇文献。
- **增量更新**：若表已存在，仅追加新文献，自动忽略重复条目。
- **列结构**（从左到右）：

  | 列 | 内容 |
  | --- | --- |
  | 文献名 | 论文标题（md 文件名去扩展名与结尾的 `_note` 后缀） |
  | 发表年份 | 只依据正文或文件名判断，不联网检索；无则留空 |
  | 期刊 | 同上，只依据正文或文件名；无则留空 |
...
- **输出**：生成 `<问题关键词>.md`，列出「可直接回答的文献」与「话题相关的文献」两类名单；同时在文献总结表中筛选呈现这些文献对应的行。若无文献总结表，会先与你确认是否需要生成。
...
- **文献名**：取 md 文件名去掉扩展名与结尾的 `_note` 后缀（`summary_note` 生成的阅读笔记统一命名为 `<论文标题>_note.md`），支持中英文混合与较长名称；去重以「论文标题（去首尾空格）」为准。
